...
//     const frontendUrl = process.env.FRONTEND_URL || 'https://college-timetable-management-system-three.vercel.app';
...
const transporter = nodemailer.createTransport(
    sgTransport({
        auth: {
            api_key: process.env.SENDGRID_API_KEY, // Set this in Render Environment Variables
        },
    })
);
...
exports.sendVerificationEmail = async (email, token) => {
    const frontendUrl = process.env.FRONTEND_URL || 'https://college-timetable-management-system-three.vercel.app';
    const url = `${frontendUrl.replace(/\/$/, '')}/verify-email/${token}`;

    try {
        await transporter.sendMail({
            from: `"AI & DS Department" <no-reply@college.com>`,
            to: email,
            subject: 'Verify your Email',
            html: `
                <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
...
exports.sendPasswordResetEmail = async (email, token) => {
    const frontendUrl = process.env.FRONTEND_URL || 'https://college-timetable-management-system-three.vercel.app';
    const url = `${frontendUrl.replace(/\/$/, '')}/reset-password/${token}`;
...
exports.sendInvitationEmail = async (email, token, role) => {
    const frontendUrl = process.env.FRONTEND_URL || 'https://college-timetable-management-system-three.vercel.app';
    const url = `${frontendUrl.replace(/\/$/, '')}/signup?invitation_token=${token}`;
    const roleName = role === 'faculty' ? 'Faculty Member' : 'Supporting Staff';
    ...
    try {
        await transporter.sendMail({
            from: `"AI & DS Department" <no-reply@college.com>`,
            to: email,
            subject: `Invitation to join AI & DS Department`,
            html: `
                <div style="font-family: Arial, sans-serif; padding: 25px; color: #1f2937;">
                    <h2 style="color: #4f46e5;">Welcome to AI & DS Department</h2>
                    <p>You have been added as a <strong>${roleName}</strong>.</p>
                    <p>Click the button below to set up your account:</p>
                    <a href="${url}" style="display:inline-block; padding:12px 28px; background-color:#4f46e5; color:white; text-decoration:none; border-radius:8px;">Create Your Account</a>
